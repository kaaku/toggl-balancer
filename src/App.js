...
class App extends Component {
  ...
  updateTimeEntries() {
    ...
    TimeEntryStore.fetchTimeEntries(startDate, endDate, apiToken).then(
      result => this.setState({ timeEntries: result }),
      error => this.setState({ error: error.toString() })
    );
  }

  render() {
    const { startDate, endDate, apiToken, timeEntries, error } = this.state;
    ...
    return (
      <React.Fragment>
        ...
        {timeEntries.length > 0 && <CalendarGrid startDate={startDate} endDate={endDate} timeEntries={timeEntries} />}
        {error && <div>{error}</div>}
      </React.Fragment>
    );
  }
